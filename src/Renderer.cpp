...
void Renderer::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "NES Emulator - Vulkan", nullptr, nullptr);
...
void Renderer::updateTexture(const uint32_t* pixels) {
    // Copy CPU pixels to Staging Buffer memory map
    VkDeviceSize imageSize = TEX_WIDTH * TEX_HEIGHT * 4;
    void* data;
    vkMapMemory(device, textureStagingBufferMemory, 0, imageSize, 0, &data);
    memcpy(data, pixels, static_cast<size_t>(imageSize));
    vkUnmapMemory(device, textureStagingBufferMemory);

    // Push buffer to Texture Image immediately using a quick command
    transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    copyBufferToImage(textureStagingBuffer, textureImage, TEX_WIDTH, TEX_HEIGHT);
...
void Renderer::createInstance() {
    ...
    appInfo.pApplicationName = "NES Emulator Vulkan";
...
void Renderer::createTextureImage() {
    ...
    createBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, textureStagingBuffer, textureStagingBufferMemory);

    createImage(TEX_WIDTH, TEX_HEIGHT, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage, textureImageMemory);
...
void Renderer::createTextureSampler() {
    ...
    samplerInfo.magFilter = VK_FILTER_NEAREST; // Point filtering for retro look
...
void Renderer::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    VkViewport viewport{};
    // Native NES internal res is 256:240. Roughly 1.0666 aspect ratio.
    // NTSC stretched makes it roughly 4:3 (1.333).
    float targetAspect = (aspectRatioMode == 0) ? (256.0f / 240.0f) : (4.0f / 3.0f);
    float windowAspect = (float)swapChainExtent.width / (float)swapChainExtent.height;

    float viewWidth = swapChainExtent.width;
    float viewHeight = swapChainExtent.height;

    if (windowAspect > targetAspect) {
        viewWidth = swapChainExtent.height * targetAspect;
    } else {
        viewHeight = swapChainExtent.width / targetAspect;
    }

    viewport.x = (swapChainExtent.width - viewWidth) / 2.0f;
    viewport.y = (swapChainExtent.height - viewHeight) / 2.0f;
...
VkPresentModeKHR Renderer::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
    if (!vSyncEnabled) {
        // Find IMMEDIATE mode for zero V-Sync lag
        for (const auto& availablePresentMode : availablePresentModes) {
            if (availablePresentMode == VK_PRESENT_MODE_IMMEDIATE_KHR) return availablePresentMode;
        }
        // If IMMEDIATE is unavailable, fallback to MAILBOX (also low latency, tears less)
        for (const auto& availablePresentMode : availablePresentModes) {
            if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) return availablePresentMode;
        }
    } else {
        // Standard V-Sync (avoids tearing, but buffers frames)
        for (const auto& availablePresentMode : availablePresentModes) {
            if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) return availablePresentMode;
        }
    }
